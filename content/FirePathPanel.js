 ...
 * 
 * FirePath is free software: you can redistribute it and/or modify it under the
 * terms of the GNU General Public License as published by the Free Software
 * Foundation, either version 3 of the License, or (at your option) any later
 * version.
 * 
 * FirePath is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR
 * A PARTICULAR PURPOSE. See the GNU General Public License for more details.
 * 
 * You should have received a copy of the GNU General Public License along with
 * FirePath. If not, see <http://www.gnu.org/licenses/>.
 */
...
FBL.ns(function() {
	with (FBL) {
		...
		// ************************************************************************************************
		// XPath Tokens from XPath V1 W3C Recommendation on
		...
		var ExprToken = "\\(|\\)|\\[|\\]|\\.\\.|@|,|::|" + NameTest + "|"
				+ NodeType + "|" + Operator + "|" + FunctionName + "|"
				+ AxisName + "|" + Literal + "|" + Numbers + "|\\.";

		var xPathToken = new RegExp(ExprToken, "g");

		var selectedToken = new RegExp(NCName + ":(?!:)|" + ExprToken + "|\\s",
				"g");
		...
		var expressionToken = new RegExp("\\]|\\)|\\*" + NameTest + "|"
				+ Literal + "|" + Numbers + "|\\.");
		...
		Firebug.FirePathPanel.prototype = extend(Firebug.Panel, {
			...
			initializeNode : function(oldPanelNode) {
				this.panelNode.addEventListener("mousedown", this.onMouseDown,
						false);
			...
			destroyNode : function() {
				this.panelNode.removeEventListener("mousedown",
			...
			show : function(state) {
				if (this.context.loaded) {
					...
					if (state) {
						...
						this.ioBox.createObjectBox(this.rootElement
			...
			showModules : function(show) {
				...
				// when there is a visibility: hidden style the collapsed
				// attribute
			...
			detach : function(oldChrome, newChrome) {
				...
				if (this.context.browser.detached
						|| (newChrome != Firebug.originalChrome && this.context == Firebug.currentContext)) {
					Firebug.FirePathPanel.LocationHighlightModule
							.addLocationListener(newChrome);
				} else {
					Firebug.FirePathPanel.LocationHighlightModule
							.removeLocationListener(oldChrome);
				}
			},
			...
			supportsObject : function(object) {
				if (object instanceof Window)
					return 2;
				else if (object instanceof Element || object instanceof Text
						|| object instanceof Attr || object instanceof Comment
			...
			getObjectDescription : function(object) {
				...
				if (object == win)
					name = $STR_XP("topWindow");
				else {
					for (var i = 0; i < win.frames.length; i++)
						if (win.frames[i].window == object) {
							name = getElementCSSSelector(object.frameElement); // +
							// "["+i+"]";
							break;
						}
				}
			...
			getOptionsMenuItems : function() {
				return [
						...
						getOptionMenu("generateAbsoluteXPathMenu",
								"generateAbsoluteXPath"),
						getOptionMenu("highlightSelectedNodes",
								"highlightNodes")];
			},
			...
			updateOption : function(name, value) {
				if (prefRegExp.test(name)) {
					...
					if (option == "highlightNodes") {
						Firebug.FirePathPanel.ResultDisplaySettingModule
								.toggleHighlight();
			...
			addStyleSheet : function(doc) {
				...
				var styleSheet = createStyleSheet(doc,
						"chrome://pageobjects/skin/FirePathPanel.css");
			...
			setResult : function(parent, result) {
				...
				this.displayResult();
				var highlightNodes = Firebug.getPref(Firebug.prefDomain,
						"firepath.highlightSelectedNodes")
				if (highlightNodes) {
					Firebug.FirePathPanel.ResultDisplaySettingModule
			...
			displayResult : function() {
				...
				if (this.evaluationResult != null
						&& this.evaluationResult.length >= 1) {
					clearNode(this.ioBoxContainer);
					delete this.ioBox.rootObjectBox;
					this.lastValidEvaluationResult = this.evaluationResult;
				}
				...
				if (this.evaluationResult != null
					...
					if (isArray(this.evaluationResult)) {
						...
						this.ioBoxContainer.textContent = type + ": "
			...
			displayLastValidResult : function() {
				this.stopLoading();

				this.setResultInfo();

				if (this.lastValidEvaluationResult.length >= 1) {
					clearNode(this.ioBoxContainer);
					delete this.ioBox.rootObjectBox;
				}

				if (this.lastValidEvaluationResult != null
						&& !this.lastValidEvaluationResult.message) {
					if (isArray(this.lastValidEvaluationResult)) {
						if (this.showDom) {
							this.ioBox.createObjectBox(this.rootElement
									|| this.location.document);
							this.ioBox
									.selectMultiple(this.lastValidEvaluationResult);
						} else
							this.ioBox
									.addObjects(this.lastValidEvaluationResult);
					} else {
						var type = typeof(this.lastValidEvaluationResult);
						type = type.charAt(0).toUpperCase() + type.substring(1);
						this.ioBoxContainer.textContent = type + ": "
								+ this.lastValidEvaluationResult;
					}
				} else if (this.showDom) {
					this.ioBox.createObjectBox(this.rootElement
							|| this.location.document);
			...
			setResultInfo : function() {
				...
				if (result == null) {
					...
					this.firePathStatusBar.setResultInfo(this.context, "error",
							$STR_XP(result.message));
				} else {
					if (FBL.isArray(result)) {
						var whiteSpace = 0;
						result.forEach(function(object) {
									if (isWhitespaceText(object))
										whiteSpace++;
								})
						switch (result.length) {
							case 0 :
								this.firePathStatusBar.setResultInfo(
										this.context, "warning",
										$STR_XP("noResultError"));
								break;
							case 1 :
								this.firePathStatusBar.setResultInfo(
										this.context, "ok",
										$STR_XP("oneResultMessage"));
								break;
							default :
								this.firePathStatusBar
										.setResultInfo(
												this.context,
												"ok",
												$STR_XP("manyResultMessage",
														result.length)
														+ (whiteSpace > 0
																? " ("
																		+ (whiteSpace == 1
						...
						switch (typeof(result)) {
							case "string" :
								this.firePathStatusBar.setResultInfo(
										this.context, "ok",
										$STR_XP("stringResultMessage"));
								break;
							case "number" :
								this.firePathStatusBar.setResultInfo(
										this.context, "ok",
										$STR_XP("numberResultMessage"));
								break;
							case "boolean" :
								this.firePathStatusBar.setResultInfo(
										this.context, "ok",
										$STR_XP("booleanResultMessage"));
								break;
						}
					}
				}
			...
			startLoading : function(objects, task) {
				...
				this.firePathStatusBar.startLoading(this.context,
						objects.length);
				this.stepper.start();
			},
			...
			onMouseDown : function(event) {
				...
				if (getAncestorByClass(event.target, "nodeTag")) {
					...
					Firebug.Inspector.highlightObject(element,
		...
		Firebug.FirePathPanel.Attr = domplate(Firebug.Rep, {
					tag : SPAN(
							...
							SPAN({
									...
									"$attr.nodeValue|filterFirePathClassValue"),
							"&quot;"),
		...
		Firebug.registerRep(Firebug.FirePathPanel.Attr,
				Firebug.FirePathPanel.Comment,
				Firebug.FirePathPanel.CDATASection);

		// ************************************************************************************************
		// Extension of FirebugReps.Element and FirebugReps.TextNode to modify
		// the
		// context menu
		// and tooltip when the FirePath tab is selected

		function overwriteMethodForPanel(object, methodName, panelName,
				newMethod) {
			...
			object[methodName] = function() {
				...
				if (selectedPanelName != panelName) {
					if (object["default" + methodName])
						return object["default" + methodName].apply(object,
								arguments);
				} else {
		...
		overwriteMethodForPanel(FirebugReps.Element, "getContextMenuItems",
				Firebug.FirePathPanel.prototype.name, function(node, target,
						context) {
					return getNodeContextMenu(node, target, context);
				});

		// Overwrite FirebugReps.Element.getTooltip
		overwriteMethodForPanel(FirebugReps.Element, "getTooltip",
				Firebug.FirePathPanel.prototype.name, function(node) {
					return getXPathFromNode(node);
				});

		// Overwrite FirebugReps.TextNode.getContextMenuItem
		overwriteMethodForPanel(FirebugReps.TextNode, "getContextMenuItems",
				Firebug.FirePathPanel.prototype.name, function(node, target,
						context) {
					return getNodeContextMenu(node, target, context);
				});
		...
		overwriteMethodForPanel(FirebugReps.TextNode, "getTooltip",
				Firebug.FirePathPanel.prototype.name, function(node) {
					return getXPathFromNode(node);
				});

		// Overwrite FirebugReps.TextNode.getRealObject
		overwriteMethodForPanel(FirebugReps.TextNode, "getRealObject",
				Firebug.FirePathPanel.prototype.name, function(object) {
		...
		Firebug.FirePathPanel.Document = domplate(FirebugReps.Document, {
			tag : DIV({
						"class" : "nodeBox containerNodeBox repIgnore",
						_repObject : "$object"
					}, DIV({
								"class" : "nodeLabel"
							}, IMG({
										...
										"class" : "nodeLabelBox repTarget"
									}, "&lt;", SPAN({
												"class" : "nodeTag"
											}, "document"), SPAN({
												"class" : "nodeBracket insertBefore"
											}, "&gt;"))), DIV({
								"class" : "nodeChildBox"
							}), DIV({
								"class" : "nodeCloseLabel"
							}, SPAN({
										"class" : "nodeCloseLabelBox repTarget"
									}, "&lt;/", SPAN({
												"class" : "nodeTag"
											}, "document"), "&gt;")))
		});
		...
		Firebug.FirePathPanel.Element = domplate(FirebugReps.Element, {
			tag : DIV(
					...
					DIV(
							...
							SPAN(
									...
									FOR(
											"attr",
											"$object|attrIterator|filterFirePathClass",
											Firebug.FirePathPanel.Attr.tag),
									SPAN({
												"class" : "nodeBracket insertBefore"
											}, "&gt;"))), DIV({
								"class" : "nodeChildBox"
			...
			filterFirePathClassValue : function(value) {
				return value.replace(" firepath-matching-node", "");
			},
			filterFirePathClass : function(attrs) {
				for (var i = 0; i < attrs.length; i++) {
					var attr = attrs[i];
					if (attr.localName == "class"
							&& attr.nodeValue == "firepath-matching-node") {
						attrs.splice(i, 1);
						break;
					}
				}
		...
		Firebug.FirePathPanel.TextElement = domplate(
				Firebug.FirePathPanel.Element, {
					tag : DIV({
								"class" : "nodeBox textNodeBox repIgnore",
								_repObject : "$object"
							}, DIV({
										"class" : "nodeLabel"
									}, SPAN(
											...
											SPAN(
													{
														"class" : "nodeText",
														_repObject : "$object.firstChild"
													},
													"$object.firstChild.nodeValue|escapeWhitespace"),
											"&lt;/", SPAN({
														"class" : "nodeTag"
													}, "$object|getTagName"),
											"&gt;"))),
		...
		Firebug.FirePathPanel.EmptyElement = domplate(
				Firebug.FirePathPanel.Element, {
					tag : DIV({
								"class" : "nodeBox emptyNodeBox repIgnore",
								_repObject : "$object"
							}, DIV({
										"class" : "nodeLabel"
									}, SPAN(
		...
		Firebug.FirePathPanel.TextNode = domplate(
				Firebug.FirePathPanel.TextElement, {
					tag : DIV({
								"class" : "nodeBox",
								_repObject : "$object"
							}, SPAN({
										"class" : "nodeText"
									}, "$object.nodeValue|escapeWhitespace"))
				});
			...
			if (node instanceof Element) {
				if (node instanceof HTMLAppletElement)
					return Firebug.FirePathPanel.EmptyElement.tag;
						...
						|| node.className == "firebugHighlight")
					return null;
		...
		function getOptionMenu(label, option) {
			...
			return {
				...
				type : "checkbox",
				checked : value,
				command : bindFixed(Firebug.setPref, Firebug,
						Firebug.prefDomain, key, !value)
			};
		}
		...
		function getNodeContextMenu(node, target, context) {
			...
			if (node != Firebug.getRepObject(target))
				return;
			...
			if (Firebug.getPref(Firebug.prefDomain,
					"firepath.showParentToolbar")) {
				var panel = context
						.getPanel(Firebug.FirePathPanel.prototype.name);
				if (panel.rootElement
						&& panel.rootElement != panel.location.document) {
					contextMenu.push({
								label : $STR_XP("copyFromContext",
										$STR_XP("xpathSelector")),
								nol10n : true,
								command : bindFixed(copyXPath, FBL, node,
										panel.rootElement)
							});
			...
			contextMenu.push({
						label : $STR_XP(
								"setSelector",
								$STR_XP(firePathBar.evaluationMode + 'Selector')),
						nol10n : true,
						command : bindFixed(firePathBar.setNode, firePathBar,
								node)
					})
			...
			if (node instanceof Element) {
				...
				if (Firebug.getPref(Firebug.prefDomain,
					...
					contextMenu.push({
								...
								nol10n : true,
								command : bindFixed(firePathBar.setContextNode,
										firePathBar, node)
							});
			...
			if (element) {
				...
				contextMenu.push({
							...
							command : bindFixed(element.scrollIntoView, element)
						});
			}
		...
		FBL.test = function() {
			var tree = window.sidebar.document.getElementById("elementList");
			var treeView = tree.view;
			var sel = treeView.selection.currentIndex; // returns -1 if the
			var fpid = treeView.getCellText(sel, tree.columns.getColumnAt(1));
			return fpid;
		}
		...
		FBL.getXPathFromNode = function(node, context) {
			var result = "";
			var stop = false;
			var absolute = Firebug.getPref(Firebug.prefDomain,
					"firepath.generateAbsoluteXPath");
		...
		FBL.getCssSelectorFromNode = function(node, context) {
			var result = '', node, parent = context || node.ownerDocument, stop = false, str;
				...
				if (node.nodeType === Node.ELEMENT_NODE) {
					if (node.id) {
						...
						stop = true;
							...
							&& node.className.replace(firepathClass, '')) {
						str = '.'
								+ node.className.replace(firepathClass, ' ')
										.trim().replace(multipleSpace, ' ')
										.split(' ').join('.');
						stop = true;
					} else {
		...
		function getNodePosition(node) {
			...
			for (var i = 0; i < siblings.length; i++) {
				...
				if (object.nodeType == node.nodeType
		...
		var InsideOutBox = Firebug.InsideOutBox
		...
		InsideOutBox.prototype = extend(InsideOutBox.prototype, {
			...
			selectMultiple : function(objects) {
				var filteredObjects = objects.filter(function(object) {
							return !isWhitespaceText(object);
						});

				this.firstSelection = true;
				this.view.startLoading(filteredObjects, bind(this.selectObject,
								this));
			},
			...
			selectObject : function(object) {
				...
				// objectBox can be null for object that have the firebugIgnore
				// attribute
				if (objectBox != null) {

					// Get the correct node in the ioBox to set its class to
					// selected
					var nodeToSelect = objectBox;

					if (object instanceof Attr) {
						// Get the nodes that contains the attributes
						var attributeNodes = getElementsByClass(objectBox,
								"nodeAttr");

						// This get the node that contains the attribute we are
						// looking for
						nodeToSelect = Array.filter(attributeNodes, function(
								attributeNode) {
							return attributeNode.childNodes[1].textContent == object.name;
						})[0];
					} else if (object instanceof Text) {
						// Get the nodes that contains the text
						var textNodes = getElementsByClass(objectBox,
								"nodeText");

						if (textNodes.length == 0) {
							this.expandObjectBox(objectBox);
							textNodes = getElementsByClass(objectBox,
									"nodeText");
						}

						// This get the node that contains the text we are
						// looking for
						nodeToSelect = Array.filter(textNodes, function(
								textNode) {
					...
					if (this.firstSelection) {
						...
						scrollIntoCenterView(nodeToSelect,
								this.view.ioBoxContainer);
					}
				}
			},
			...
			addObject : function(object) {
				...
				if (object instanceof Attr) {
					...
					// return the node that contains the attribute we are
					// looking for
					var attr = Array.filter(attributeNodes, function(
							attributeNode) {
						// the node containing the attribute name
						return attributeNode.childNodes[1].textContent == object.name;
					})[0];
					if (attr)
						setClass(attr, "selected");
				} else {
		...
		Firebug.FirePathPanel.xPathAutoCompleter = function(evaluator) {
			this.autoCompleter = new Firebug.AutoCompleter(null, bind(
							this.getAutoCompleteRange, this), bind(
							this.getAutoCompleteList, this), true, true);
		...
		Firebug.FirePathPanel.xPathAutoCompleter.prototype = {
			...
			getAutoCompleteRange : function(value, offset, context) {
				...
				if (tokens.length == 0) {
					...
					end = (firstTokenIndex == -1 ? value.length : start
							+ firstTokenIndex);
				}
			...
			getAutoCompleteList : function(preExpr, expr, postExpr, context) {
				...
				if (!tokens.length) {
					// At the begining
					returning = extendArrays(["/"], this.getNodeList(expr,
									inside, tokens, true)
									|| [], this.Functions);
				} else if (tokens[0] == "|") {
					...
					returning = this.getNodeList(expr, inside, tokens, true)
						...
						||
						// If the token is * or name operator, we make sure it's
						// actually an operator
						(tokens[0] != "*"
								&& !include(this.NameOperators, tokens[0]) && include(
								this.Operators, tokens[0]))
						|| ((tokens[0] == "*" || include(this.NameOperators,
								tokens[0]))
								&& tokens[1] != undefined && !include(
								extendArrays(["@", "::", "(", "["],
										this.Operators), tokens[1]))) {
					// After "(", "," "[" or an operator
					var nodeList = this.getNodeList(expr, inside, tokens, true);
					if (nodeList)
						returning = extendArrays(["/"], nodeList,
								this.Functions);
				...
				if (tokens[0] == "." && returning.length > 0
						&& !include(returning, "."))
					returning.unshift(".");
				if (tokens[0] == "/" && returning.length > 0
						&& !include(returning, "/"))
			...
			getNodeList : function(expr, inside, tokens, addAxes) {
				...
				if (endsWith(flattenedExpr, "@")
						|| endsWith(flattenedExpr, "attribute::")) {
					...
					this.addNamespacesAndSort(returning);
					if (this.evaluator.hasResult(flattenedExpr + "comment()"))
						returning.push("comment()");
					if (this.evaluator.hasResult(flattenedExpr + "node()"))
						returning.push("node()");
					if (this.evaluator.hasResult(flattenedExpr + "text()"))
						returning.push("text()");
					if (addAxes) {
						var parentExpression = flattenedExpr.replace(/\/$/, "");
						parentExpression += (endsWith(parentExpression, "/")
								? "descendant-or-self::node()"
								: "");
						parentExpression = parentExpression || "/";
						result = this.evaluator
								.evaluateExpression(parentExpression);
						if (result) {
							var hasSelf = result.some(function(node) {
										return node.nodeType == 1;
									});
							var hasAttribute = result.some(function(node) {
										return node.hasAttributes();
									});
							var hasAncestor = result.some(function(node) {
										return node.parentNode
												&& node.parentNode.nodeType == 1;
									});
							var hasDescendant = result.some(function(node) {
										return node.hasChildNodes();
									});
							var hasFollowingSibling = result.some(
									function(node) {
										return hasNextSibling(node);
									});
							var hasFollowing = hasFollowingSibling
									|| result.some(function(node) {
												return hasFollowingNode(node);
											});
							var hasPrecedingSibling = result.some(
									function(node) {
										return hasPreviousSibling(node);
									});
							var hasPreceding = hasPrecedingSibling
									|| result.some(function(node) {
												return hasPrecedingNode(node);
											});
							returning.push(".");
							if (hasAttribute)
								returning.push("@");
							if (hasAncestor)
								returning.push("..");
							if (hasAncestor)
								returning.push("ancestor::");
							if (hasSelf || hasAncestor)
								returning.push("ancestor-or-self::");
							if (hasDescendant)
								returning.push("descendant::");
							if (hasSelf || hasDescendant)
								returning.push("descendant-or-self::");
							if (hasFollowing)
								returning.push("following::");
							if (hasFollowingSibling)
								returning.push("following-sibling::");
							if (hasAncestor)
								returning.push("parent::");
							if (hasPreceding)
								returning.push("preceding::");
							if (hasPrecedingSibling)
								returning.push("preceding-sibling::");
							if (hasDescendant)
								returning.push("child::");
							if (hasSelf)
								returning.push("self::");
							if (hasAttribute)
								returning.push("attribute::");
			...
			getOperators : function(token, inside) {
				...
				if ((/\)|"[^"]*"|'[^']*'|\d+(?:\.\d?)?|\.\d+/).test(token))
					...
					returning = extendArrays(this.NodeSetOperators,
			...
			flattenExpression : function(tokens) {
				...
				while (token = clone.shift()) {
					...
					if (inPredicate == 0
							...
							&& (include(["(", ",", "["], token) ||
									// If the token is * or name operator, we
									// make sure it's
									// actually an operator
									(token != "*"
											&& !include(this.NameOperators,
													token) && include(
											this.NumberAndBooleanOperators,
											token)) || ((token == "*" || include(
									this.NameOperators, token))
									&& clone[0] != undefined && !include(
									extendArrays(["@", "::", "(", "["],
											this.Operators), clone[0])))) {
			...
			NumberAndBooleanOperators : ["and ", "or ", "< ", "<= ", "> ",
					">= ", "= ", "+ ", "- ", "* ", "div ", "mod "],

			Operators : ["and", "or", "<", "<=", ">", ">=", "=", "+", "-", "*",
					"div", "mod", "/", "//", "|"],

			NameOperators : ["and", "or", "div", "mod"],

			Functions : ["concat(", "substring(", "substring-after(",
					"substring-before(", "string(", "local-name(", "name(",
					"namespace-uri(", "normalize-space(", "translate(",
					"ceiling(", "count(", "floor(", "last()", "number(",
					"position()", "round(", "sum(", "boolean(", "contains(",
					"false()", "lang(", "not(", "starts-with(",
					"string-length(", "true()"]
		}
		...
		Stepper.prototype = {
			...
			execute : function() {
				try {
					for (var i = 0, date = new Date(); i < this.timePeriod; i = Date
							.now()
							- date) {
		...
		Firebug.FirePathPanel.LocationHighlightModule = extend(Firebug.Module,
				{
					...
					addLocationListener : function(chrome) {
						chrome.$("fbLocationList").addEventListener(
								"mouseover", this.onLocationMouseOver, false);
						chrome.$("fbLocationList").addEventListener("mouseout",
								this.onLocationMouseOut, false);
					},

					removeLocationListener : function(chrome) {
						chrome.$("fbLocationList").removeEventListener(
								"mouseover", this.onLocationMouseOver, false);
						chrome.$("fbLocationList").removeEventListener(
								"mouseout", this.onLocationMouseOut, false);
					},
					...
					onLocationMouseOver : function(event) {
						...
						if (panel.name == panelName) {
							...
							if (repObject && repObject.frameElement) {
								Firebug.Inspector.highlightObject(
										repObject.frameElement,
										Firebug.currentContext);
							}
						}
					},
		...
		Firebug.FirePathPanel.ResultDisplaySettingModule = extend(
				Firebug.Module, {
					initialize : function() {
					 ...
					 * this.highlightButton = Firebug.chrome
					 * .$("FirePathBarHighlightButton"); },
					 * 
					...
					reattachContext : function(browser, context) {
						this.ShowDOMButton = Firebug.chrome
								.$("FirePathBarShowDOMButton");
					},

					showContext : function(browser, context) {
						this.ShowDOMButton = Firebug.chrome
								.$("FirePathBarShowDOMButton");
						// this.refreshShowDOMButton(context.getPanel(panelName));
					},
					...
					showPanel : function(browser, panel) {
						if (panel) {
							if (panel.name == panelName) {
								this.ShowDOMButton.collapsed = false;
								// this.ShowDOMButton.setAttribute("disabled",
								// true);
								if (!panel.context.firePathResultNotHighlighted)
									this.highlight(panel.context);
							} else {
								this.ShowDOMButton.collapsed = true;
								this.clear();
							}
						}
					},
					...
					refreshHighlight : function(panel) {
						this.ShowDOMButton.setAttribute("disabled", false);
						// this.ShowDOMButton.setAttribute("checked", true);
						if (!panel.context.firePathResultNotHighlighted) {
							this.clear();
							this.highlight(panel.context);
						}
					...
					toggleShowDOM : function() {
						var panel = Firebug.currentContext.getPanel(panelName);
						var buttonStatus = panel.showDom;
						panel.showDom = !buttonStatus;

						if (panel.evaluationResult != null) {
							if (panel.evaluationResult.length >= 1) {
								panel.displayResult();
							} else {
								panel.displayLastValidResult();
							}
							this.ShowDOMButton.setAttribute("checked",
									panel.showDom);
						}
					},
					...
					highlight : function(context) {
						...
						var isCSSAttached = doc
								.getElementById("firepath-matching-node-style");

						if (!isCSSAttached) {
							var head = doc.getElementsByTagName("head")[0]
							if (head) {
								if (!context.firePathStyleSheet) {
									var styleSheet = document.createElementNS(
											xhtmlNS, "style");
									styleSheet.firebugIgnore = true;
									styleSheet.setAttribute("type", "text/css");
									styleSheet.setAttribute("id",
											"firepath-matching-node-style");
									styleSheet.innerHTML = ".firepath-matching-node {"
								...
								// if we can't attach the stylesheet there is no
								// need to
						...
						this.stepper = new Stepper(
								arrayIterator(resultElement), step, end);
						this.stepper.start();
					},

					clear : function(doc) {
						// clear the stylesheet from the doc before evaluating
						// an
						// expression
						if (doc) {
							var styleSheet = doc
		...
		Firebug.registerModule(Firebug.FirePathPanel.LocationHighlightModule,
				Firebug.FirePathPanel.ResultDisplaySettingModule);
		Firebug
				.registerUIListener(Firebug.FirePathPanel.ResultDisplaySettingModule);
	}
});
