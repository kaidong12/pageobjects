...
var localFileUtils = {
	...
	openURLInputStream : function(url) {
		...
		var stream = ioService.newChannelFromURI(ioService.newURI(url, null,
				null)).open();
		var sis = Components.classes['@mozilla.org/scriptableinputstream;1']
				.createInstance(Components.interfaces.nsIScriptableInputStream);
		sis.init(stream);
	...
	getFileSeperator : function(localFile) {
		if (Components.interfaces.nsILocalFileWin
				&& localFile instanceof Components.interfaces.nsILocalFileWin) {
			return '\\';
		}
		return '/';
	},

	// Samit: Enh: Work around the nsILocalFile.appendRelativePath() restriction
	// of not being able to use relative paths with ".." and "."
	...
	createFolder : function(base, dir) {
		Components.utils.import("resource://gre/modules/FileUtils.jsm");
		var folder = FileUtils.getDir(base, [dir], true);
		return folder;
	},
